...
optimize_dns() {
    log_install "dns-optimization" "dnsmasq split DNS"

    # Skip if dnsmasq split DNS is already active
    if grep -q "^nameserver 127.0.0.1" /etc/resolv.conf 2>/dev/null && pidof dnsmasq >/dev/null 2>&1; then
        log_info "dnsmasq split DNS already active"
        return 0
    fi

    # Measure DNS latency
    local dns_time
    dns_time=$(curl -w "%{time_namelookup}" -o /dev/null -s --connect-timeout 10 https://api.anthropic.com 2>/dev/null)
    ...
    # Check if DNS is slow (threshold: 1 second, awk avoids bc dependency)
    if awk "BEGIN {exit !($dns_time <= 1.0)}" 2>/dev/null; then
        log_info "DNS resolution is fast (${dns_time}s). No optimization needed."
        return 0
    fi

    log_warn "dns" "DNS resolution slow (${dns_time}s), setting up split DNS..."
    ...
    while IFS= read -r line; do
        case "$line" in
            ...
            search\ *)
                search_line="$line"
                local IFS=' '
                read -ra domains <<< "${line#search }"
                search_domains+=("${domains[@]}")
                ;;
        esac
    done < /etc/resolv.conf

    # If no split DNS needed (no custom NS + search domain pair), simple prepend
    if [[ ${#search_domains[@]} -eq 0 || ${#custom_ns[@]} -eq 0 ]]; then
        ...
        log_success "dns-optimization" "Google DNS prepend (${dns_time}s → ${new_time}s)"
        return 0
    fi

    # Install dnsmasq if needed
    if ! command -v dnsmasq >/dev/null 2>&1; then
        log_install "dnsmasq" "apt"
        if DEBIAN_FRONTEND=noninteractive apt-get install -y -qq dnsmasq >/dev/null 2>&1; then
            log_success "dnsmasq" "apt"
        else
            log_error "dnsmasq" "apt"
            ...
            cp "${tmp}.new" /etc/resolv.conf
            rm -f "$tmp" "${tmp}.new"
            log_warn "dns" "Fell back to Google DNS prepend"
            return 0
        fi
    fi

    # Stop existing dnsmasq (may have auto-started after install)
    kill "$(pidof dnsmasq)" 2>/dev/null
    sleep 0.5

    # Backup original resolv.conf (only first time)
    [[ ! -f /etc/resolv.conf.pre-dnsmasq ]] && cp /etc/resolv.conf /etc/resolv.conf.pre-dnsmasq

    # Generate dnsmasq split-dns config
    local conf="/etc/dnsmasq.d/split-dns.conf"
    mkdir -p /etc/dnsmasq.d
    {
        echo "# Split DNS (generated by optimize_dns)"
        echo "no-resolv"
        echo "server=8.8.8.8"
        echo "server=8.8.4.4"
    ...
    # Start dnsmasq with our config only
    if dnsmasq -C "$conf" 2>/dev/null; then
        local new_time=$(curl -w "%{time_namelookup}" -o /dev/null -s --connect-timeout 10 https://api.anthropic.com 2>/dev/null)
        log_success "dns-optimization" "dnsmasq split DNS (${dns_time}s → ${new_time}s)"
    else
        # Revert on failure
        cp /etc/resolv.conf.pre-dnsmasq /etc/resolv.conf 2>/dev/null
        log_error "dns-optimization" "dnsmasq failed to start, reverted"
        return 1
    fi
}
